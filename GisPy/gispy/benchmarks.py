...
class Wolfe:
    ...
    def wolfe_eval(input_file, prefix_names, variables, doc_filter, use_wolfe_vars=False, use_gispy_vars=False):
        ...
        df['zero'] = 0
        df['zgis'] = st.zscore(df['gis'].astype(float), nan_policy='omit')
        ...
        ttest_result = st.ttest_ind(a=list(df_yes['zgis']), b=list(df_no['zgis']), equal_var=True)

        results = {'mean_gist_yes': statistics.mean(list(df_yes['zgis'])),
                   'mean_gist_no': statistics.mean(list(df_no['zgis'])),
    ...
    def run_benchmark(self, gispy_indices_file: str, prefix_names: dict, doc_filter=[], use_wolfe_vars=False,
        ...
        results = pd.DataFrame(
            columns=['vars_name', 'mean_gist_yes', 'mean_gist_no', 'distance', 'ttest_statistic',
                     'ttest_pvalue'])
        plot_id = 0
        for i, vars_dict in enumerate(vars_dicts):
            print(f'Config {i+1}/{len(vars_dicts)}...')
        ...
        results.sort_values('ttest_statistic', ascending=False, inplace=True)
